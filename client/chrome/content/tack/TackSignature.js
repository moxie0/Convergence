...
function TackSignature(secItem) {
  ...
  const SIG_MIN_GENERATION_LENGTH = 1;
  const SIG_GENERATION_LENGTH     = 1;
  const SIG_EXPIRATION_LENGTH     = 4;
  const SIG_TARGET_LENGTH         = 32;
  const SIGNATURE_LENGTH          = 64;

  const TACK_SIG_LENGTH   = SIG_TYPE_LENGTH + SIG_MIN_GENERATION_LENGTH +
                            SIG_GENERATION_LENGTH + SIG_EXPIRATION_LENGTH + 
                            SIG_TARGET_LENGTH + SIGNATURE_LENGTH;

  const SIG_TYPE_OFFSET           = 73;
  const SIG_MIN_GENERATION_OFFSET = SIG_TYPE_OFFSET + SIG_TYPE_LENGTH;
  const SIG_GENERATION_OFFSET     = SIG_MIN_GENERATION_OFFSET + SIG_MIN_GENERATION_LENGTH;
  const SIG_EXPIRATION_OFFSET     = SIG_GENERATION_OFFSET + SIG_GENERATION_LENGTH; 
  const SIG_TARGET_OFFSET         = SIG_EXPIRATION_OFFSET + SIG_EXPIRATION_LENGTH;
  ...
  this.minGeneration = data[SIG_MIN_GENERATION_OFFSET];
